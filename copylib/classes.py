...
class Node:
        xmlFieldMap = open(os.path.join(resource_path,'templateField.xml'), 'r').read()
        xmlListMap = open(os.path.join(resource_path,'templateList.xml'), 'r').read()
        ...
        def xmlMap(self):
            if self.occurrency and self.occurrency > 1:
                ...
                d = {
                    ...
                    'io': self.io,
                    'name': name,
                    'mapperClassPath': packageName + '.' + serviceName + 'MQMapper',
                    'others': others
                }
...
class CodeTree():
    ...
    templateConnector = open(os.path.join(resource_path,'templateConnector.xml'), 'r').read()
    mockResponse = open(os.path.join(resource_path,'mockResponse.txt'), 'r').read()[:-1]
    templateConnectorImpl = open(os.path.join(resource_path,'TemplateConnectorImpl.java'), 'r').read()
    templateConnectorInterface = open(os.path.join(resource_path,'TemplateConnectorInterface.java'), 'r').read()
    ...
    def xmlMap(self):
        ...
        d = {
        'mapping': mapping,
        'connectorName': lowFirst(serviceName) + 'Connector',
        'moduleName': lowFirst(serviceName) + 'ConnectorModule',
        'beanName': lowFirst(serviceName) + 'ConnectorBean',
        'interfaceClassPath': packageName + '.I' + serviceName + 'Connector',
        'implClassPath': packageName + '.' + serviceName + 'ConnectorImpl',
        }
