...
parser.add_option("-c", "--configuration", dest="json",
                  help="Read configuration from JSON configuration file, this configuration will override all the others")
...
class PlaceHolder(Node):
    ...
    cursorParser = re.compile('.*-NUMOCCURS(-\w+)?$|.*-NUM-ELEMENTI$')
    ...
    def __init__(self, name, dataType, parent=None):
        ...
        #     pdb.set_trace()
        if m:
            self.intSize = ((m.group(4) and (int(m.group(4))) - 1) or 0) + (
            (m.group(2) and len(m.group(2)) > 0 and len(m.group(2))) or 0)
            self.decSize = ((m.group(7) and (int(m.group(7))) - 1) or 0) + (
            (m.group(5) and len(m.group(5)) > 0 and len(m.group(5))) or 0)
